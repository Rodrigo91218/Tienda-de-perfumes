...
class DetalleFactura(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    pedido_id = db.Column(db.Integer, db.ForeignKey('factura.id'), nullable=True)
    cantidad = db.Column(db.Integer, nullable=False)
...
def datos_cliente():
    if request.method == 'POST':
        ...
        return redirect(url_for('descargar_factura_pdf', factura_id=nueva_factura.id))

    return render_template('datos_cliente.html')



@app.route('/descargar_factura_pdf/<int:factura_id>')
def descargar_factura_pdf(factura_id):
    # Recuperar los datos de la factura
    factura = db.session.query(Factura, Cliente).join(Cliente, Cliente.id == Factura.pedido_id).filter(Factura.id == factura_id).first()
    if not factura:
        return "Factura no encontrada", 404
    detalles_pedido = db.session.query(DetalleFactura).filter(DetalleFactura.pedido_id == factura.Factura.pedido_id).all()

    # Crear un objeto en memoria para almacenar el PDF
    buffer = BytesIO()
    p = canvas.Canvas(buffer, pagesize=letter)
    
    # Escribir el contenido en el PDF
    p.drawString(100, 750, f"Factura ID: {factura.id}")
    p.drawString(100, 730, f"Fecha: {factura.fecha}")
    p.drawString(100, 710, f"Cliente: {factura.Cliente.nombre} {factura.Cliente.apellido}")
    p.drawString(100, 690, f"Total: ${factura.total}")

    # Añadir detalles de los productos
    y_position = 670
    for detalle in detalles_pedido:
        p.drawString(100, y_position, f"Producto: {detalle.producto.nombre} - Cantidad: {detalle.cantidad} - Precio: ${detalle.precio_unitario}")
        y_position -= 20
    ...
    # Ir al inicio del buffer y devolver el PDF como respuesta
    buffer.seek(0)
    return Response(buffer, mimetype='application/pdf', headers={'Content-Disposition': f'attachment; filename=factura_{factura_id}.pdf'})
